...
pub async fn get_file(id: i64, path: &str, req: &HttpRequest) -> HttpResponse {
    ...
    let mut res = match req.cookie("token") {
        Some(c)
            if let Some(account) = intererr!(Account::find_by_token(c.value()).await)
                && account.id == id =>
        {
        ...
        _ => {
            if intererr!(Visibilities::visibility(&pathbuf).await).visibility
                == ItemVisibility::Private
                || !intererr!(fs::try_exists(&pathbuf).await)
            {
                return intererr!(NamedFile::open_async(IMG_NOT_FOUND.get().unwrap()).await)
                    .into_response(req);
            } else {
